...
const authenticatedNavListTemplate = (userInfo) => `
  <ul class="nav justify-content-center align-items-center d-flex flex-column flex-md-row">
    <li class="nav-item">
    ...
    <li id="userLoggedMenu" class="nav-item dropdown">
      ...
      <ul class="dropdown-menu dropdown-menu-end">
        <a id="userLogOut" href="#/logout" class="dropdown-item">Logout</a>
      </ul>
    </li>
  </ul>
`;

const noteItemTemplate = (note, user, bookmarkButton) => {

  const btn = note.owner === user.name ?
    `<button
      id="deleteNoteButton"
      data-id="${note.id}"
